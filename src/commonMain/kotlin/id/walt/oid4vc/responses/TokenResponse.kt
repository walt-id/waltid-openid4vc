...
data class TokenResponse private constructor(
  ...
  companion object: JsonDataObjectFactory<TokenResponse>() {
    ...
    private val knownKeys = setOf("access_token", "token_type", "expires_in", "refresh_token", "vp_token", "scope", "c_nonce", "c_nonce_expires_in", "authorization_pending", "interval", "presentation_submission", "error", "error_description", "error_uri")
    fun fromHttpParameters(parameters: Map<String, List<String>>): TokenResponse {
      return TokenResponse(
        ...
        parameters.filter { !knownKeys.contains(it.key) && it.value.isNotEmpty() }.mapValues { Json.parseToJsonElement(it.value.first()) }
      )
    }
  }
